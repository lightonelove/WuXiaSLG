...
public class TargetedIndicatorAutoSetup
{
    ...
    private static void ProcessPrefabForTargetedIndicators(GameObject prefabRoot, string context)
    {
        ...
        {
            Debug.Log($"[TargetedIndicatorAutoSetup] {context}: Processing {indicators.Length} TargetedIndicator(s) in prefab: {prefabRoot.name}");
            ...
            foreach (TargetedIndicator indicator in indicators)
            {
                if (indicator != null)
                {
                    ...
                    if (currentEntity == null)
                    {
                        ...
                        anyChanges = true;
                        Debug.Log($"[TargetedIndicatorAutoSetup] Auto-found CombatEntity for {indicator.gameObject.name} in prefab {prefabRoot.name}");
                    }
                    else
                    {
                        Debug.Log($"[TargetedIndicatorAutoSetup] CombatEntity already assigned for {indicator.gameObject.name} in prefab {prefabRoot.name}");
                    }
                }
            }
            ...
            if (anyChanges)
            {
                ...
                Debug.Log($"[TargetedIndicatorAutoSetup] Updated CombatEntity references in prefab: {prefabRoot.name}");
            }
        }
    }
}
...
public class TargetedIndicatorEditor : Editor
{
    public override void OnInspectorGUI()
    {
        ...
        EditorGUILayout.Space();
        EditorGUILayout.HelpBox("CombatEntity reference will be automatically found when prefab is saved or scene is saved.", MessageType.Info);
