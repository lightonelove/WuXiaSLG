...
namespace Wuxia.GameCore
{
    public class SLGCoreUI : MonoBehaviour
    {
        ...
        [Header("Custom Cursor")] public Image cursorImage;
        public bool enableCustomCursor = true;
        public Vector2 cursorOffset = Vector2.zero;

        [Header("Floor Mouse Indicator")] public GameObject floorIndicatorPrefab; // FloorIndicator prefab
        public LayerMask floorLayerMask = -1; // Floor層級遮罩
        public bool enableFloorIndicator = true;

        [Header("Action Buttons")] public GameObject actionButtonsPanel; // 按鈕面板
        public Button moveButton; // 移動按鈕
        [Header("技能按鈕")] public List<Button> skillButtons; // 技能按鈕列表 (A/B/C/D)

        [Header("Button Colors")] public Color normalButtonColor = Color.white;
        ...
        private void InitializeActionButtons()
        {
            ...
            if (moveButton != null)
            {
                moveButton.onClick.AddListener(() => { ToggleActionMode(CharacterCore.PlayerActionMode.Move); });
            }
            else
            {
            }
            ...
            for (int i = 0; i < skillButtons.Count; i++)
            {
                ...
                {
                    int index = i; // 捕獲迴圈變數
                    skillButtons[i].onClick.AddListener(() => { ExecuteSkillButton(index); });
                }
            }
        ...
        private void ExecuteSkillButton(int skillIndex)
        {
            ...
            // 檢查對應技能是否可用
            bool canUse = currentCharacter.skillsComponent != null &&
                          currentCharacter.skillsComponent.CanUseSkillByIndex(skillIndex);
            CombatSkill skill = currentCharacter.skillsComponent != null
                ? currentCharacter.skillsComponent.GetSkill(skillIndex)
                : null;
        ...
        {
            if (character == null || character.skillsComponent == null ||
                character.skillsComponent.currentSelectedSkill == null)
                return;
